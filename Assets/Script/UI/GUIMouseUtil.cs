...
// 헬퍼 — 여러 곳이 각자 동일한 로직을 중복 구현하고 있던 것을 통합. 포인터 좌표는 GameInputScheme
// (입력 스킴 중앙화)을 통해 읽는다.
public static class GUIMouseUtil
{
    ...
    // 선택이 각자 동일한 3항 판정을 반복하던 것을 통합했다. InputManager는 BuildingControlPanel
    // 확인이 하나 더 필요해서(OnGUI라 IsPointerOverGameObject로 안 잡힘) 이 공통부에 자기 조건을
    // 추가로 ||한다 — 여기서는 항상 확인해야 하는 3개만 다룬다.
