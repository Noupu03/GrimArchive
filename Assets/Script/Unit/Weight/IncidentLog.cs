// 이벤트 발생 1건의 기록 ("영수증"). 유닛이 사건을 겪을 때마다 하나씩 쌓이고, 웨이브 종료 시
// 생존자들의 IncidentEntry를 모아 6장 생존자 전역 반영 계산의 입력으로 쓴다.
// EventID: 연산공식 문서 3장의 이벤트 종류 ID (WeightEnums.EventId). IncidentID: 실제로 발생한
// "사건" 하나를 가리키는 고유 ID(10장/6-2장) — 같은 사건을 여러 유닛이 동시에 겪었을 때 이 값으로
// 묶는다. 호출부에서 발급/전달한다.
