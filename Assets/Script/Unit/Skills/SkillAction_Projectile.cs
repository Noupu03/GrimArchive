...
public class SkillAction_Projectile : SkillAction
{
    ...
    public override void Execute(Unit unit, Unit target, float minDist)
    {
        ...
        System.Action updateThreatAction = () =>
        {
            ...
            {
                Hitbox maxHitbox = BuildRectHitboxWithAngle(unit, threat.width, maxRange, unit.GetComponent<CombatStateComponent>().State.currentAttackAngle);
                ...
                if (!_d.isPiercing)
                {
                    ...
                    Vector2 forward = new Vector2(Mathf.Cos(unit.GetComponent<CombatStateComponent>().State.currentAttackAngle), Mathf.Sin(unit.GetComponent<CombatStateComponent>().State.currentAttackAngle));
                ...
                unit.GetComponent<AIStateComponent>().currentThreat.hitbox = BuildRectHitboxWithAngle(unit, threat.width, threat.depth, unit.GetComponent<CombatStateComponent>().State.currentAttackAngle);
            }
        };
    ...
    private void FireProjectile(Unit attacker, int maxDistance)
    {
        ...
        Hitbox projectileHitbox = new Hitbox
        {
            ...
            rotation = attacker.GetComponent<CombatStateComponent>().State.currentAttackAngle * Mathf.Rad2Deg
        };

        Vector2 moveDir = new Vector2(Mathf.Cos(attacker.GetComponent<CombatStateComponent>().State.currentAttackAngle), Mathf.Sin(attacker.GetComponent<CombatStateComponent>().State.currentAttackAngle));
