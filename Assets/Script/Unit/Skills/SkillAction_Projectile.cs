...
public class SkillAction_Projectile : SkillAction
{
    ...
    public override void Execute(Unit unit, Unit target, float minDist)
    {
        ...
        System.Action updateThreatAction = () =>
        {
            ...
            {
                Hitbox maxHitbox = BuildRectHitboxWithAngle(unit, threat.width, maxRange, unit.CombatState.currentAttackAngle);
                ...
                if (!_d.isPiercing)
                {
                    ...
                    Vector2 forward = new Vector2(Mathf.Cos(unit.CombatState.currentAttackAngle), Mathf.Sin(unit.CombatState.currentAttackAngle));
                ...
                unit.currentThreat.hitbox = BuildRectHitboxWithAngle(unit, threat.width, threat.depth, unit.CombatState.currentAttackAngle);
            }
        };
    ...
    private void FireProjectile(Unit attacker, int maxDistance)
    {
        ...
        Hitbox projectileHitbox = new Hitbox
        {
            ...
            rotation = attacker.CombatState.currentAttackAngle * Mathf.Rad2Deg
        };

        Vector2 moveDir = new Vector2(Mathf.Cos(attacker.CombatState.currentAttackAngle), Mathf.Sin(attacker.CombatState.currentAttackAngle));
