...
public abstract class SkillAction
{
	...
	// ─── 타겟팅 두 축 (자세한 설명은 SkillTargeting.cs) ──────
	...
	/// 직접 찾아 돌려준다. 반환값이 그대로 CanExecuteAgainst/Execute로 전달된다.
	/// </summary>
	public virtual Unit ResolveTarget(Unit unit, Unit nearestEnemy) => nearestEnemy;

	/// <summary>
	/// 지금 이 스킬을 실제로 쓸 수 있는 위치인가 — 세 FSM 상태가 스킬 실행 직전에 부르는 공통
	/// 관문이다(개별 게이트 로직 금지). target은 ResolveTarget이 돌려준 "이미 해결된 대상"이다.
	/// </summary>
	...
	// 대부분의 IsAvailable override가 앞머리에 반복하던 쿨다운 게이트 — 각 서브클래스가 SkillData(_d)를 개별 소유해 cooldownSlot을 파라미터로 받는다.
		...
		AttackShape  shape = AttackShape.Melee)
	{
		...
		if (castMs <= 0f)
		{
			...
			try
			{
				effectAction?.Invoke();
				attackAction?.Invoke();
				unit.TriggerAttackVisibilityBoost(); // 공격 후 가시성 상승, 재공격 시 지속시간 초기화
		...
		{
			// 시전 상태(isCastingAttack/castTimer/castingUnits/pendingCastUpdate)는 위 통지 직전에 이미 세팅했다.
	...
	// Unit이 ScriptableObject라서 List<Unit>.Contains()는 네이티브 유효성 검사가 딸린 Equals를 호출한다
	// — Footprint 중복 셀 제거용 Contains를 HashSet 병행 관리로 O(1) 조회로 대체한다.
	...
	// 특정 대상 포함 여부만 확인하던 자리를 위한 전용 진입점 — _hitboxQueryResultSet을 재사용해
	...
	/// </summary>
	// onHit — 대상별 피해 적용 직후(스턴 포함) 호출되는 선택적 콜백. GetEnemiesInHitbox를 밖에서 따로
	// 다시 호출하면 그 사이 죽은 대상이 두 번째 조회에서 걸러져 킬 데미지 히트에 이펙트가 안 뜬다.
	...
	// 항상 최대 감소를 받는다. 조정하려면 이 상한(전역) 또는 skills.json + 프리팹의 baseCooldown(개별)을 바꾼다.
