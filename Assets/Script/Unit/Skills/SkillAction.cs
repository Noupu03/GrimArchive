...
public abstract class SkillAction
{
	...
	/// <summary>
	/// 이 스킬이 실제로 겨눌 대상을 결정한다. AI는 "가장 가까운 적"만 알아서, 아군 대상 스킬은 여기서
	/// 직접 찾아 돌려준다(진영 축 소비 지점). 반환값이 그대로 CanExecuteAgainst/Execute로 전달된다.
	/// </summary>
	public virtual Unit ResolveTarget(Unit unit, Unit nearestEnemy) => nearestEnemy;

	/// <summary>
	/// 지금 이 스킬을 실제로 쓸 수 있는 위치인가 — CombatFSMState/StandGroundAttackFSMState/
	/// PlayerCommandFSMState 세 곳이 스킬 실행 직전에 부르는 공통 관문이다(개별 게이트 로직 금지).
	/// target은 ResolveTarget이 돌려준 "이미 해결된 대상"이다(적일 수도 아군일 수도 있다).
	/// </summary>
	...
	// 대부분의 IsAvailable override가 앞머리에 반복하던 쿨다운 게이트. 각 서브클래스가 SkillData(_d)를
	// 개별 소유해(베이스 클래스엔 없음) cooldownSlot을 파라미터로 받는다.
		...
		AttackShape  shape = AttackShape.Melee)
	{
		...
		// 시전 상태를 통지보다 먼저 세팅해야 한다 — 위협 타일 표시 시간(GameSession의 OnThreatCreated
		// 구독)이 공격자의 castTimer를 읽어 결정되므로, 순서가 반대면 항상 0.5초 폴백이 쓰인다.
		if (castMs > 0f)
		{
	...
	// Unit이 ScriptableObject라서 List<Unit>.Contains()는 원소마다 네이티브 유효성 검사가 딸린 Equals를
	// 호출한다 — Footprint 중복 셀 제거용 Contains를 HashSet 병행 관리로 O(1) 조회로 대체한다.
	...
	// CombatFSMState/PlayerCommandFSMState가 GetEnemiesInHitbox(...).Contains(target)로 특정 대상
	// 포함 여부만 확인하던 자리를 위한 전용 진입점 — _hitboxQueryResultSet을 재사용해 List.Contains()의
	// 선형 Equals 호출을 피한다. GetEnemiesInHitbox와 동일하게 호출 직후 즉시 소비할 것.
	...
	/// </summary>
	// onHit — 대상별 피해 적용 직후(스턴 포함) 호출되는 선택적 콜백. GetEnemiesInHitbox를 이 메서드
	// 밖에서 따로 다시 호출하면 그 사이 죽은 대상이 두 번째 조회에서 걸러져 킬 데미지 히트에 이펙트가
	// 안 뜨는 문제가 생긴다 — 같은 순회에서 함께 처리해야 한다.
	...
	// 항상 최대 감소를 받아 모든 스킬 쿨다운이 절반이 된다. 조정하려면 이 상한(전역) 또는
	// skills.json + 프리팹의 baseCooldown(개별)을 바꾼다.
