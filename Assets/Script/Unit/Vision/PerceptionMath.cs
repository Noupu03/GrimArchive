...
// 인지·정보판정·실패처리 시스템의 순수 계산 함수 모음 — VisionMath(01/01-A)와 동일 컨벤션(부수효과
// 없음, 저장/조회는 Unit/UnitFunction 담당). 문서가 위임한 부분은 CLAUDE.md 관례대로 스텁 처리한다.
public static class PerceptionMath
{
	...
	// ─────────────────────────── 9장. 정신력 보정 (인류 전용) ───────────────────────────
	// 문서는 6단계 보정값만 명시하고 구간 경계는 밝히지 않는다 — 기존 가중치 시스템의 MentalErrorState와
	// 별개로 0~100%를 6등분(17/33/50/67/83)해서 매핑한다. ratio가 높을수록 상위 단계(흥분 쪽)로 간다.
	...
	// 인류 전용(몬스터는 사용하지 않음). maxMental이 0 이하면 안정(0) 취급한다 — 안 그러면 0으로 나눠 항상 100%가 되어 최고 단계 보너스가 공짜로 붙는다.
	...
	// ─────────────────────────── 8장. 최종 계산 가시성 ───────────────────────────
	// 최종 계산 가시성 = 대상 가시성(VisionMath.FinalVisibility) + 경계 반영 감지 보정 + 정신력 보정(인류 관찰자만 0이 아님).
	...
	// ─────────────────────────── 12장. 인지 결과 확률표 ───────────────────────────
	// 현재 확률표는 문서가 명시한 임시 기준 그대로다. "100 이상" 구간은 "80 이상"과 분리된 별도 행 — 하한과 대칭되는 상한 처리가 명시적으로 존재한다.
	...
	// roll01: 0~1 균등 난수는 호출부(UnitFunction)가 넘겨준다 — 순수 함수로 유지해 경계값을 테스트로 고정 검증할 수 있게 한다.
	...
	// 전파·소리·간접입력 시스템이 아직 없어 실제 발생 지점이 없다 — 값만 미리 계산해 둔다.
	...
	// "선택"해서 행동으로 옮기는 건 아직 없는 문서의 몫 — 여기서는 후보 목록만 돌려준다.
...
// 정확 인지 대상 분류. 함정/건물은 아직 전용 엔티티가 없어 실질적으로 EnemyUnit/Corpse/WipeoutTrace만
// 실제로 발생한다 — 나머지는 대응 엔티티가 생기면 그대로 쓸 수 있도록 미리 분류만 남겨둔다.
