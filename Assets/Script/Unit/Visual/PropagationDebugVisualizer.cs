...
// 임시 디버그 시각화(07_전파·소리·간접입력 확인용) — 검증 끝나면 이 파일+GameCompositionRoot 등록+
// GameSession 호출부만 지우면 제거된다. ThreatTileRenderer와 동일 컨벤션(매 프레임 Render(units),
// 즉석 머티리얼이 이 URP에서 안 보여 기본 머티리얼 사용). 살아있는 인류마다 전파 범위를 원으로 그리고,
// OnSoundEmitted(발생 사건 자체, 감지 여부 무관)를 구독해 소리 위치에 종류별 색 원을 표시했다 지운다.
public class PropagationDebugVisualizer
{
	// DebugInfoPanel의 "시야 표시" 토글과 동일 컨벤션 — 기본은 꺼짐, 우측 상단 패널에서 항목별로 켠다.
	...
	// 링 텍스처 자체의 반지름(스프라이트 로컬 단위) — 실제 표시 반지름(타일 수)은 transform.localScale로 맞춘다(scale 1 = 반지름 1칸).
	...
	// 리스트 인덱스로 풀에서 원을 배정받으면 프레임마다 구성이 바뀌며 같은 GameObject가 다른 플래시로
	// 재배정되는 버그가 생긴다 — 각 플래시가 발생 즉시 자기 전용 GameObject를 만들어 소멸까지 들고 있는다.
	...
	public void Construct(UnitGenerate unitGenerate)
	{
		...
		// OnSoundPerceived(감지 성공)가 아니라 OnSoundEmitted(발생 사건 자체)를 구독 — 토글이 꺼져 있으면 콜백에서 바로 걸러낸다.
	...
	{
		// 위치·반경은 발생 시점에 이미 고정해뒀고, 여기선 매 프레임 남은 시간에 따라 투명도만 갱신한다.
	...
	// 별도 에셋 없이 코드로 원형 링 텍스처를 한 번만 생성해 재사용 — 즉석 Shader.Find 머티리얼은 이 URP 프로젝트에서 안 보여 SpriteRenderer 기본 머티리얼을 쓴다.
