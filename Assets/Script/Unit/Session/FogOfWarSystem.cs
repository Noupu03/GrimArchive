...
// 오버레이라 게임플레이 로직은 건드리지 않는다(Room.FogRevealed는 시각 전용 플래그).
public class FogOfWarSystem
{
    ...
    // ── 안개 ──────────────────────────────────────────────────────────
    // 방/게이트 하나당 Tilemap 2장(배경/무늬)으로 일괄 렌더링 — 타일별 GameObject 방식은 큰 방에서 프레임 드랍을 유발해 청크 단위 드로우콜로 대체했다.
    private readonly Dictionary<Room, GameObject> _roomFogVisuals = new Dictionary<Room, GameObject>();
    // 문/통로 안개 — 게이트 하나는 두 방을 잇는 통로라 어느 한 쪽 Room에도 배타적으로 속하지 않아 (floorIndex, min, max)로 별도 추적한다.
    ...
    // Tilemap이 공유하는 타일 애셋 2장(배경/무늬) — 모든 안개 스폰이 이 둘만 재사용한다. TryPrepareFogTiles가 지연 생성.
    private UnityEngine.Tilemaps.Tile _fogBackingTile;
    private UnityEngine.Tilemaps.Tile _fogPatternTile;
	// 런타임 생성 스프라이트가 이 빌드에서 렌더링되지 않는 문제 우회: 안개를 Lit 셰이더로 바꾸고 벽+안개 통합 셰도우 캐스터로 빛을 차단해 항상 검정이 되게 한다.
	private Material _fogMaterial;
    private const float FogFadeOutSeconds = 0.6f;
    // 위협타일(ThreatTileRenderer)보다 확실히 위여야 유닛/오브젝트/라벨이 안개 아래로 실제로 안 보인다. 배경+무늬 2겹이라 배경=이 값, 무늬=+1.
    private const int FogSortingOrder = 1000;
    // obj/fog.png 단독으로는 줄무늬 틈으로 안이 비쳐서, 뒤에 불투명에 가까운 단색 배경을 깔고 위에 무늬를 얹는 2겹 구조로 바꿨다.
    ...
    public void Initialize()
    {
        ...
        // 0층 최좌측 숨은 스폰 청크에 예외적으로 하드코딩 안개를 씌운다 — 어떤 Reveal 트리거에도 등록되지 않아 영원히 안 걷힌다.
        ...
        // 문/통로 안개 + 빈 청크 안개 처리 — 모든 Room.FogRevealed가 확정된 뒤 실행해야 양옆 판정이 정확하다.
        for (int floorIndex = 1; floorIndex < cmap.map.floors.Length; floorIndex++)
        {
    ...
    // SpawnObject의 단색 폴백 텍스처 생성과 동일한 방식 — 불투명에 가까운 배경 한 장을 미리 만들어 모든 안개 타일이 공유한다.
    private Sprite EnsureFogBackingSprite()
    {
        if (_fogBackingSprite != null) return _fogBackingSprite;

		// 런타임 생성 Texture2D 스프라이트가 이 빌드에서 렌더링 안 되는 문제를 피해 엔진 내장 whiteTexture를 재사용한다.
    ...
    // 안개 타일 애셋(배경/무늬) 지연 생성 — 최초 1회만 만들고 이후 모든 안개 스폰이 공유한다. 스프라이트 원본 크기가 1 월드유닛과 안 맞을 수 있어 Tile.transform으로 셀 크기에 맞춘다.
    private bool TryPrepareFogTiles()
    {
        ...
        // 배경(불투명에 가까움)이 무늬 텍스처의 줄무늬 틈으로 안이 비쳐 보이지 않게 먼저 완전히 가리고, 무늬(위)는 obj/fog.png 그대로.
    ...
    // 안개 덩어리(배경+무늬 Tilemap 2장) 생성 — 여러 Spawn* 메서드 공용. parentGroup이 이미 floorOffset만큼
    // 옮겨져 있어 원본 월드 좌표를 그대로 셀 좌표로 쓸 수 있다. 셰도우 캐스터는 여기서 안 붙이고 별도로 만든다.
    ...
    private void SpawnFogForRoom(Room room)
    {
        ...
        for (int x = room.Bounds.xMin; x < room.Bounds.xMax; x++)
        {
            ...
            {
                // 청크 전체를 가린다(벽 타일 포함). 문/통로 타일만 예외 — SpawnFogForGate가 두 방의 안개 상태를 함께 보고 따로 처리한다.
                var p2 = new Vector2Int(x, y);
                if (doorTiles.Contains(p2)) continue;
                // Bounds는 사각 경계값이라 방이 L자 등 비직사각형이면 다른 방 타일까지 포함할 수 있어 roomGrid로 실제 소유 방을 재확인한다.
    ...
    // 게이트가 잇는 두 방 중 하나라도 FogRevealed==false면 그 게이트의 문 타일 전체를 안개로 덮는다. 방이 없는 쪽은 이미 걷힌 것으로 간주한다.
    ...
    // roomId < 0인 빈 청크는 어떤 Room에도 속하지 않아 SpawnFogForRoom에 안 걸리고, 유닛이 못 들어가 해제 트리거도 없어 영구 안개로 둔다.
    ...
    // 벽 없는 구간에서 경계가 끊어진다. 안개는 방이 걷힐 때마다 바뀌므로 벽과 달리 매번 층 전체를 재계산한다.
    private bool[,] BuildStillFoggedMask(int floorIndex, int worldW, int worldH)
    {
        ...
        // 0층은 Room.FogRevealed가 항상 true라 방 기반 판정만으로는 숨은 청크가 안 걷힌 것으로 안 잡혀 여기서 강제로 덮어쓴다.
        if (floorIndex == 0)
        {
        ...
        // 게이트(문) 타일은 두 방 중 하나라도 안 걷혔으면 안개 — 청크의 개별 방 판정과 별개로 덮어쓴다(SpawnFogForGate와 동일한 규칙).
        if (floor.gates != null)
        {
    ...
    // 새 ShadowCaster2D는 Update()가 한 번 돌아야 셰도우 그룹에 등록되므로, 기존 캐스터를 먼저 지우면
    // 빛이 새는 프레임이 생긴다 — 새 캐스터를 먼저 만들고 2프레임 뒤에 기존 걸 지운다.
    ...
    // 안개 해제 — UnitFunction.SyncRoomAffiliation이 방 최초 입장을 감지하면 호출한다. FogRevealed는 한번 true가 되면 다시 false로 안 돌아간다(영구 해제).
    ...
    // '방 최초 진입' 트리거는 이동 제약 때문에 드물게만 발생해 안전장치로만 남겨두고, 더 확실한
    // '점령'(OffenseProcessor가 RoomFaction을 Player로 확정하는 순간)을 주 트리거로 쓴다. 점령된 방과 Gate로 직접 연결된 인접 방까지만 안개를 걷는다.
    ...
    // room이 걷혔을 때 맞닿은 게이트 중 반대쪽 방도 이미 걷혀 있으면 그 게이트 안개도 같이 걷는다.
    ...
    // SceneTransitionFade.FadeAsync와 동일한 UniTask 알파 페이드 관례. Tilemap.color(전체 셀 곱연산
    // 틴트)만 1→0으로 보간하면 baked 알파에 자동으로 곱해져 시작 알파를 따로 기억할 필요가 없다.
    ...
    // ── 횃불 ──────────────────────────────────────────────────────────
    // Torch.prefab을 Instantiate — 0층도 다른 층과 동일한 청크 단위 배치를 받는다. 청크당 "게이트로
    // 뚫리지 않은 완전히 막힌 벽 1면"을 랜덤으로 골라 그 앞 바닥 칸에 놓으며(벽/게이트 판정은
    // CreateMap에 위임), 그런 벽이 없는 청크는 배치하지 않는다.
    public void SpawnTorches()
    {
        ...
        for (int floorIdx = 0; floorIdx < cmap.map.floors.Length; floorIdx++)
        {
            ...
            for (int cx = 0; cx < w; cx++)
            {
                for (int cy = 0; cy < h; cy++)
                {
                    // 0층 숨은 스폰 청크 — 상시 안개로 덮여 안 보이는 데다, 횃불 빛이 안개 경계 너머로 새는 것도 막는다.
    ...
    // 횃불 지연 스폰 전용 — RevealRoomFog가 room을 막 걷었을 때 호출되며, 쌓여있던 대기 좌표를 지금 실제로 Instantiate한다.
    ...
    // 청크 4면 중 게이트로 뚫리지 않은 완전히 막힌 벽만 후보로 모아 랜덤 선택 후 그 앞 바닥 칸을 반환 — 벽/게이트 판정은 CreateMap에 위임한다.
    ...
    private GameObject SpawnTorchAt(int floorIdx, Vector2Int tilePos, TorchWallSide side)
    {
        ...
        // 방향별 스프라이트/위치 보정은 TorchVisual.cs가 담당 — 자식 "Visual"의 로컬 좌표에만 적용한다(루트를 옮기면 Light2D도 같이 밀려 엉뚱한 곳을 비춘다).
