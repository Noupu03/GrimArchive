...
public class GameSession : NativeRoutine, IOffenseQuery
{
    ...
    // 몬스터 배치 프리셋(2026-08-19 재구현) — "0층에 인류가 소환된 시점부터" 저장된 디펜스 시작
    // 위치가 있는 플레이어 몬스터를 그 위치로 이동시키고 소집 대기에 들어가게 한다. HumanWaveManager.
    // PreSpawnWaveUnits(사전 스폰 경로)와 StartWave의 즉시 스폰 폴백 경로 둘 다에서 호출된다 — "0층
    // 인류 소환" 이벤트가 그 두 지점 중 하나로만 일어나기 때문. 실제 이동은 우클릭 이동과 동일한
    // 경로(playerMoveTarget/PlayerCommandFSMState)를 재사용한다 — 위치 도착 이후의 실제 전투 행동은
    // 기존 몬스터 AI를 그대로 따른다(이 기능은 전투 AI를 바꾸는 시스템이 아니다).
    //
    ...
    public void ApplyMonsterDefenseStartPositions()
    {
        ...
        foreach (var unit in units)
        {
            ...
            {
                unit.isMustered = true;
                ApplyDefenseFacingDirection(unit); // 제자리 소집도 배치된 몬스터와 동일하게 방향을 맞춘다.
                inPlaceCount++;
            }
    ...
    // 몬스터 배치 프리셋(2026-08-19 재수정, 사용자 요청 "1. 시작방에서 소집 시, 해당 층의 계단
    // 방향을 바라봐야 함. 2. 시작방 이외의 방에서 소집시, 시작방으로 향하는 문 방향을 바라봐야 함")
    // — 소집 이동이 끝나 제자리에 서는 순간 PlayerCommandFSMState.CompletePlayerCommand가 호출한다.
    // 시작방은 인류가 이 층에 처음 올라오는 계단(이전 층으로 내려가는 계단) 쪽을, 그 외의 방은
    // 시작방으로 가는 최단 경로상 첫 번째 문(=이 방에 침입자가 들어올 때 반드시 지나야 하는 문)을
    // 바라보게 한다.
    public void ApplyDefenseFacingDirection(Unit unit)
    {
        if (unit == null || cmap == null) return;
        if (!roomGrid.TryGetValue(new Vector3Int(unit.position.x, unit.position.y, unit.currentFloor), out Room room)) return;

        Vector2Int target;
        bool found;
        if (TryGetStartRoomId(unit.currentFloor, out int startRoomId) && room.RoomId == startRoomId)
        {
            // 시작방 — 이 층으로 올라오는 계단(이전 층으로 내려가는 계단) 방향을 바라본다.
            found = cmap.TryGetStairApproachPosition(unit.currentFloor, unit.currentFloor - 1, unit.position, out target)
                || cmap.TryGetStairPosition(unit.currentFloor, unit.currentFloor - 1, out target);
        }
        else
        {
            found = TryGetDefenseFacingDoorTile(unit.currentFloor, room.RoomId, out target);
        }
    ...
    private bool TryGetStartRoomId(int floorIndex, out int startRoomId)
    {
        startRoomId = -1;
    ...
    private bool TryGetDefenseFacingDoorTile(int floorIndex, int roomId, out Vector2Int doorTile)
    {
        doorTile = default;
        if (cmap == null || cmap.map.floors == null || floorIndex < 0 || floorIndex >= cmap.map.floors.Length) return false;
        ...
        if (!TryGetStartRoomId(floorIndex, out int startRoomId)) return false;
        ...
        var doorTiles = GetGateDoorTiles(doorGate);
