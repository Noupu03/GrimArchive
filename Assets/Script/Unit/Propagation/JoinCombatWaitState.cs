...
// 07문서 7장(전투 진입 시 전파 예외) + 07-A 9장(적 인지 후 전투 시작·합류 판정)의 하위 상태. 위험도
// 2단계 이상 + 비근거리(>2칸)에서 적을 정확 인지하면 발견자는 즉시 전투 대신 합류 의사 응답(2초)과
// 실제 합류(최대 5초)를 기다린다. AlertSearchState와 같은 패턴 — Human.currentJoinCombatWait가 null이
// 아닌 동안만 의미가 있고, 더 높은 우선순위(플레이어 명령 등)가 끼어들면 그냥 버려진다.
