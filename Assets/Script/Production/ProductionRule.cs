...
public class ProductionRule : ScriptableObject
{
    ...
    // 플레이어(몬스터 진영) 생산 건물이 제공하는 기본 생산 규칙 목록 — B키 배치(InputManager)와 게임
    // 시작 시 자동 배치(GameSession) 둘 다 이 팩토리를 써서 두 경로의 값이 어긋나지 않게 한다.
    // Knight/Archer는 Human 기본 진영이라 플레이어 생산 건물에서 뽑으면 안 되고, 지금 코드베이스에
    // 있는 플레이어용 Monster UnitType은 MeleeTank뿐이라 우선 이것만 등록한다.
