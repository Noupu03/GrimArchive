...
// personalMap(개인 지도: 타일 위험도, 오브젝트, 몬스터 목격, 방 위험도/흥미도) 요약을, Monster면
// monsterMap(2026-08-20 신규, 지형+함정 기록만, 가중치 없음) 요약을 보여준다.
// "각 유닛 오브젝트에서 지도를 볼 수 있게" 하기 위한 디버그 뷰 — 씬에서 유닛을 선택하면 바로 보인다.
...
public class UnitVisualEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        if (visual.boundUnit is Human human)
        {
        ...
        else if (visual.boundUnit is Monster monster)
        {
            // 몬스터는 지형+함정만 기록하는 축소판(MonsterMapKnowledge) — 위험도/흥미도/방/오브젝트
            // 흥미도 등 인류 전용 항목은 애초에 없다(사용자 요청 "가중치 X").
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("개인 지도 (MonsterMapKnowledge)", EditorStyles.boldLabel);
            EditorGUILayout.TextArea(monster.monsterMap.BuildDebugSummary(), GUILayout.MinHeight(80));

            EditorGUILayout.Space();
            EditorGUILayout.LabelField("지형 밝히기 (흰색=바닥, 회색=벽, 검정=미탐색)", EditorStyles.boldLabel);
            foreach (int floor in monster.monsterMap.KnownTerrainFloors)
            {
                var tex = monster.monsterMap.GetTerrainTexture(floor);
                if (tex == null) continue;
        ...
        {
            EditorGUILayout.HelpBox("이 유닛은 개인 지도를 갖지 않습니다.", MessageType.None);
            return;
        }
