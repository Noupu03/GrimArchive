...
2026-07-31 07_전파·소리·간접입력 시스템 구현으로 발동 지점이 생겼다(아래 "전파·소리·간접입력 시스템"
섹션 참고) — 다만 E_HIT_HEAVY_INDIRECT/E_MONSTER_KILL_INDIRECT를 실제 소리 확인 결과와 연결하는
마지막 배선은 아직 남아있다(시야인지반응_구현현황 최신본의 "다음 우선순위 1번" 참고). 여전히 막힌
것은 **버프/디버프/소환**, **함정/방어건물**, **오브젝트 "조사" 단계** 등 "이벤트를 발생시킬 하위
게임 시스템 자체가 없는" 경우들 — 최신 구현현황 문서의 "다음 우선순위" 섹션 참고.
...
구현 현황은 `Assets/문서/구현현황/구현중/시야인지반응_구현현황_2026-07-31.txt`(01/01-A/02문서 —
...
1. `Assets/문서/구현현황/구현중/시야인지반응_구현현황_2026-07-31.txt`(또는 그 이후 최신본) 맨 위
...
구현 현황은 `Assets/문서/구현현황/구현중/시야인지반응_구현현황_2026-07-31.txt`(01/01-A/02문서와 같은
...
Assets/Script/Unit/Propagation/
  ...
  PropagationSystem.cs     — 부수효과 있는 호출부(소리 이벤트 버스/감지·선택/전파 조건/합류 판정/
                              상호작용 정보 전파) — PartyDeathSystem과 동일 성격
Assets/Script/Unit/Core/PropagationComponent.cs — 유닛별 저장소(PerceptionComponent와 동일 패턴)
Assets/Tests/PropagationSystemTests.cs — 07/07-A 숫자 예시 고정 테스트
```
...
TrapPass`(함정작동음)에서 발생하고, `UnitFunction.OnUpdate`의 기존 0.1초 틱에서 인류마다 감지·선택
(`PropagationSystem.TickSoundPerception`)한다. 반응은 기존 `AlertSearchState`를 확장해 재사용한다
(`IsSoundResponse` 등 필드 추가 — 03문서가 이미 `IsDeathSearch`로 같은 방식을 썼던 전례를 따름).
전투 진입 시 위험도 2단계 이상 + 비근거리(>2칸)면 `TacticalBehaviorType.JoinCombatWait`(신규, Panic
바로 다음 우선순위)로 즉시 전투 대신 합류 대기를 거친다 — `CombatFSMState.GetPriority`가 진입점.

### 다음에 이 시스템을 확장할 때

1. `시야인지반응_구현현황_2026-07-31.txt`(또는 그 이후 최신본)의 "07문서 장별 구현 현황" → "다음
   우선순위 구현 후보" 순으로 확인. 특히 1번(E_HIT_HEAVY_INDIRECT/E_MONSTER_KILL_INDIRECT를 실제
   소리 확인 결과와 연결하는 배선)이 가중치 시스템 쪽 "다음 우선순위 4번"과 동일한 작업이라 우선순위가
   높다.
